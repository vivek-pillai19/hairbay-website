...
const FranchiseList = ({ franchises, selectedId, onSelect, onAddNew }) => {
  ...
  const filteredFranchises = franchises
    ...
    .sort((a, b) => {
      if (locationSort === 'AtoZ') return a.city.localeCompare(b.city);
      if (locationSort === 'ZtoA') return b.city.localeCompare(a.city);
      return 0;
    });
  ...
  return (
    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl overflow-hidden shadow-sm flex flex-col h-full animate-in fade-in slide-in-from-left duration-700">
      ...
      <div className="p-6 border-b border-slate-100 dark:border-slate-800 space-y-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-bold text-slate-900 dark:text-white uppercase tracking-wider">Franchise Network</h3>
          </div>
          <div className="flex items-center gap-2">
            <button 
          ...
          <div className="flex gap-2">
            <div className="relative group min-w-[110px] flex-1">
              ...
              <select 
                ...
                className="w-full pl-9 pr-8 py-2 bg-slate-100 dark:bg-slate-800 border-none rounded-xl text-xs focus:ring-2 focus:ring-primary text-slate-900 dark:text-white transition-all outline-none appearance-none font-bold cursor-pointer"
              >
            ...
            <div className="relative group min-w-[110px] flex-1">
              ...
              >
                ...
                <option value="AtoZ">City (A-Z)</option>
                <option value="ZtoA">City (Z-A)</option>
              </select>
              <ChevronDown className="w-3 h-3 absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none" />
            </div>
          </div>
        ...
        <div className="divide-y divide-slate-100 dark:divide-slate-800">
          {filteredFranchises.map((franchise) => (
            <FranchiseCard 
              key={franchise.id} 
              {...franchise} 
              isSelected={selectedId === franchise.id}
              onEdit={() => onSelect(franchise)}
            />
          ))}
          {filteredFranchises.length === 0 && (
            <div className="py-20 text-center flex flex-col items-center justify-center opacity-40 animate-in fade-in zoom-in duration-500">
               ...
               <button 
                onClick={() => { setSearchTerm(''); setStatusFilter('All'); setLocationSort('None'); }}
      ...
      <div className="p-4 border-t border-slate-100 dark:divide-slate-800 bg-white dark:bg-slate-900 text-center">
         <p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em]">Showing {filteredFranchises.length} Registry Entries</p>
      </div>
    </div>
  );
};
